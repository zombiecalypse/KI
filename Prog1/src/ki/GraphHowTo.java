...
public class GraphHowTo {
	public static Graph livingRoomGraph() {
		Graph room =  new HashGraph(new UGraphElementFactory());
		Vertex[][] map = new Vertex[14][14];
		for (int row = 0; row < 14; row++)
			for (int col = 0; col < 14; col++) {
				map[row][col] = room.addVertex();
				map[row][col].setAttribute("x", new Float(col*30));
				map[row][col].setAttribute("y", new Float(row*30));
			}
		for (int row = 0; row < 13; row++) {
			for (int col = 0; col < 13; col++) {
				if (9 <= col && col < 12 && 5 <= row && row < 8) continue;
				if (10 <= row && 7 <= col && col <= 10) continue;
				if (Math.abs(row-4)+Math.abs(col-4) < 4) continue;
				
				room.addEdge(map[row][col], map[row][col+1]);
				room.addEdge(map[row][col], map[row+1][col]);
				room.addEdge(map[row+1][col], map[row+1][col+1]);
				room.addEdge(map[row][col+1], map[row+1][col+1]);
			}
		}
		room.addEdge(map[13][13], map[12][13]);
		room.addEdge(map[13][13], map[13][12]);
		map[0][1].setAttribute("isStart", new Boolean(true));
		map[13][12].setAttribute("isGoal", new Boolean(true));
		return room;
	}
  ...
  public static void main(String[] args) {
    ...
    String inputGraphFile = "/home/aaron/tst.glx";
    searchGraph = livingRoomGraph(); //readGraph(inputGraphFile);
    ...
    if (searchGraph != null) {
      ...
      try {
        ...
        IGraphSearch mSearch = new GraphSearch();
        ...
        catch (NoStartVertex e) {
        	System.out.println("No Start Vertex!");
        	return;
        } catch (NoEndVertex e) {
        	System.out.println("No End Vertex!");
        	return;
		}

        // R�ckgabe des Resultates
        Vertex[] pathToGoalNode;
        try {
        	 pathToGoalNode = mSearch.getSolution();
        }
        catch (NoStartVertex e) {
        	System.out.println("No Start Vertex");
        	return;
